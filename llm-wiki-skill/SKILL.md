...
2. **保存原始素材**到 `raw/` 对应目录：
   ...
   **图片理解流程**（默认已安装 MiniMax MCP `understand_image` 工具时启用，**仅在图片数 ≥ 2 张时触发**）：
   保存素材后，扫描内容中是否包含图片引用（`![` 或 `<img` 或 `.png`/`.jpg`/`.gif`/`.svg` URL）。
   - 如果只有 **1 张图片** → 跳过图片理解，不调用 MCP
   - 如果有 **≥2 张图片** → 进入以下流程：
     1. 对每个图片引用，提取文件名，在知识库根目录下的 `raw/05_Assets/` 目录查找对应文件
     2. 找到的图片，调用 `mcp__MiniMax__understand_image` 理解内容
     3. 所有图片理解结果汇总为隐式上下文，在 Step 1 中传入 LLM（不写入任何文件）
     4. Step 1 / Step 2 处理完成后，图片描述直接丢弃，不保留在任何地方
   - 图片理解不阻塞主流程——如果 MiniMax MCP 不可用，跳过此步，素材仍正常消化
   - frontmatter `image_paths` 记录格式不变（见下方简化处理），供参考

   示例隐式上下文（仅传入 LLM，不落盘）：
   ```
   配图说明：
   - fig1.png：展示了 Multi-Head Attention 计算流程...
   - arch.jpg：Transformer 整体编码器-解码器结构图...
   ```
...
5. **Step 1：结构化分析**：
   - 输入：原始内容 + `purpose.md` + 现有 wiki 结构（至少读取 `index.md` 概要）+ 图片理解结果（如有）
...
6. **Step 2：页面生成**：
   - 输入：原始内容 + `purpose.md` + Step 1 的分析结果 + 现有相关 wiki 页面 + 图片理解结果（如有）
...
1. **保存原始素材**到对应 `raw/` 目录
   - **图片理解**（仅在图片数 ≥ 2 张时触发）：同完整处理流程，扫描图片引用后在 `raw/05_Assets/` 查找；找到的调用 `mcp__MiniMax__understand_image`；描述作为隐式上下文传入 LLM 后直接丢弃；frontmatter `image_paths` 字段记录图片路径供参考；单张图片时跳过此步
2. **读取上下文并检查缓存**：
   - 仍然优先读取 `purpose.md`
...
7. **向用户报告结果**：
   ...
   **zh**：
   ```
   已删除：
     - raw/03_Resources/2024-01-15-ai-article.md
