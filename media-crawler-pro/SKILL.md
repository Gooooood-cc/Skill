...
For each record:
  ...
  ├─ If video type: download video → Bcut ASR transcribe
  ├─ Generate Markdown with YAML frontmatter
  ├─ Write to 03_Resources/{nickname}_{date}_{title}.md
  └─ UPDATE is_synced=1
  ↓
Optional: add_punctuation_by_llm.py
```
...
Video notes are auto-transcribed during sync.
...
2. If video: download audio and transcribe via Bcut ASR
...
### 4.5 Add Punctuation (All Platforms)

```bash
cd g:/MediaCrawlerPro/MediaCrawlerPro-Python && uv run python scripts/add_punctuation_by_llm.py
```

Uses MiniMax LLM (`MiniMax-M2.7`) to add natural punctuation to all transcripts in `03_Resources/`.
